...
class PBE(PDE_utils):
    ...
    def border_value(self,x,y,z):
        ...
        return (1/(4*self.pi*self.epsilon_2))*sum - self.G(x,y,z)
...
class Helmholtz(PDE_utils):
    ...
    def residual_loss(self,mesh,model,X,SU):
        ...
        u = model(R)
        r = self.laplacian(mesh,model,X) - self.kappa**2*(u+self.G(x,y,z))
        Loss_r = tf.reduce_mean(tf.square(r))
        return Loss_r  
...
class Non_Linear(PDE_utils):
    ...
    def residual_loss(self,mesh,model,X,SU):
        ...
        R = mesh.stack_X(x,y,z)
        u = model(R)
        r = self.laplacian(mesh,model,X) - self.kappa**2*tf.math.sinh(u+self.G(x,y,z))     
        Loss_r = tf.reduce_mean(tf.square(r))
        return Loss_r
