...
class Mesh():
    ...
    def create_mesh(self,borders,ins_domain):
        ...
        xspace = np.array(range(1,int(self.ub[0])+1),dtype=self.DTYPE)
        yspace = np.array(range(1,int(self.ub[0])+1),dtype=self.DTYPE)
        zspace = np.array(range(1,int(self.ub[0])+1),dtype=self.DTYPE)
        X, Y, Z = np.meshgrid(xspace, yspace, zspace)
        
        self.X_r = tf.constant(np.vstack([X.flatten(),Y.flatten(), Z.flatten()]).T)
